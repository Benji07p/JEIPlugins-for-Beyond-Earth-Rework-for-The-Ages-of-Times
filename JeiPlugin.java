...
public class JeiPlugin implements IModPlugin {
	...
	//Custom
	private List<Fluid> customTagFluids;

	//Water
	private List<Fluid> waterTagFluids;

	//Lava
	private List<Fluid> lavaTagFluids;

	//Hydrogen
	private List<Fluid> hydroTagFluids;
	...
	public void registerRecipes(IRecipeRegistration registration) {
		...
		//Custom
		this.customTagFluids = this.generateCustomTagFluids();

		//Water
		this.waterTagFluids = this.generateWaterTagFluids();

		//Lava
		this.lavaTagFluids = this.generateLavaTagFluids();

		//Hydrogen
		this.hydroTagFluids = this.generateHydroTagFluids();
		...
		registration.addRecipes(RocketTier1JeiCategory.recipeType, generateHydroLoadingRecipes());
		// Lava Tier 2
		registration.addRecipes(RocketTier2JeiCategory.recipeType, generateLavaLoadingRecipes());
		// Water Tier 3
		registration.addRecipes(RocketTier3JeiCategory.recipeType, generateWaterLoadingRecipes());
		// Custom Tier 4
		registration.addRecipes(RocketTier4JeiCategory.recipeType, generateCustomLoadingRecipes());
		// Rover
	...
	private List<Fluid> generateFuelTagFluids() {
		return ForgeRegistries.FLUIDS.getValues().stream().filter(f -> f.isSource(f.defaultFluidState()) && Methods.tagCheck(f, TagsRegistry.FLUID_FUEL_TAG)).collect(Collectors.toList());
	}
	...
	// Custom Loading
	private List<Fluid> generateCustomTagFluids() {
		return ForgeRegistries.FLUIDS.getValues().stream().filter(f -> f.isSource(f.defaultFluidState()) && Methods.tagCheck(f, TagsRegistry.OXYGENE_FLUID_TAG)).collect(Collectors.toList());
	}

	private List<FuelLoadingRecipe> generateCustomLoadingRecipes() {
		List<ItemStack> customTagBuckets = new ArrayList<>();

		for (Fluid fluid : this.customTagFluids) {
			customTagBuckets.add(new ItemStack(fluid.getBucket()));
		}

		FuelLoadingRecipe recipe = new FuelLoadingRecipe(customTagBuckets, this.customTagFluids);
	...
	// Water Loading
	private List<Fluid> generateWaterTagFluids() {
		return ForgeRegistries.FLUIDS.getValues().stream().filter(f -> f.isSource(f.defaultFluidState()) && Methods.tagCheck(f, TagsRegistry.WATER_FLUID_TAG)).collect(Collectors.toList());
	}

	private List<FuelLoadingRecipe> generateWaterLoadingRecipes() {
		List<ItemStack> waterTagBuckets = new ArrayList<>();

		for (Fluid fluid : this.waterTagFluids) {
			waterTagBuckets.add(new ItemStack(fluid.getBucket()));
		}

		FuelLoadingRecipe recipe = new FuelLoadingRecipe(waterTagBuckets, this.waterTagFluids);
	...
	// Lava Loading
	private List<Fluid> generateLavaTagFluids() {
		return ForgeRegistries.FLUIDS.getValues().stream().filter(f -> f.isSource(f.defaultFluidState()) && Methods.tagCheck(f, TagsRegistry.LAVA_FLUID_TAG)).collect(Collectors.toList());
	}

	private List<FuelLoadingRecipe> generateLavaLoadingRecipes() {
		List<ItemStack> lavaTagBuckets = new ArrayList<>();

		for (Fluid fluid : this.lavaTagFluids) {
			lavaTagBuckets.add(new ItemStack(fluid.getBucket()));
		}

		FuelLoadingRecipe recipe = new FuelLoadingRecipe(lavaTagBuckets, this.lavaTagFluids);
	...
	// Hydrogen Loading
	private List<Fluid> generateHydroTagFluids() {
		return ForgeRegistries.FLUIDS.getValues().stream().filter(f -> f.isSource(f.defaultFluidState()) && Methods.tagCheck(f, TagsRegistry.HYDROGEN_FLUID_TAG)).collect(Collectors.toList());
	}

	private List<FuelLoadingRecipe> generateHydroLoadingRecipes() {
		List<ItemStack> hydroTagBuckets = new ArrayList<>();

		for (Fluid fluid : this.hydroTagFluids) {
			hydroTagBuckets.add(new ItemStack(fluid.getBucket()));
		}

		FuelLoadingRecipe recipe = new FuelLoadingRecipe(hydroTagBuckets, this.hydroTagFluids);
